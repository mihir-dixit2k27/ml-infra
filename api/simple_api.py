...
model_loader = ModelLoader(model_name=MODEL_NAME, tracking_uri=MLFLOW_TRACKING_URI, model_alias=MODEL_ALIAS)
...
def get_db_connection(retries=5, delay=3):
    ...
    for i in range(retries):
        try:
            ...
            logger.warning(f"DB connection failed (attempt {i+1}/{retries}). Error: {e}")
            if i < retries - 1:
...
async def load_model():
    ...
    logger.info(f"--- Attempting to load model '{MODEL_NAME}' alias '{MODEL_ALIAS}' at startup ---")
    logger.info(f"--- Using MLflow Tracking URI: {MLFLOW_TRACKING_URI} ---")
    logger.info(f"--- Will retry up to {retries} times with {delay}s delay ---")
    
    for attempt in range(retries):
        try:
            logger.info(f"--- [Attempt {attempt + 1}/{retries}] Connecting to MLflow and loading model... ---")
            ...
            try:
                ...
                try:
                    ...
                    model_version = client.get_model_version_by_name(MODEL_NAME, MODEL_ALIAS)
                    CURRENT_MODEL_VERSION = model_version.version
                    logger.info(f"--- Fetched production model version: {CURRENT_MODEL_VERSION} ---")
                except Exception:
                    ...
                    if latest_versions:
                        ...
                        logger.info(f"--- Fetched latest model version: {CURRENT_MODEL_VERSION} ---")
                    else:
                        CURRENT_MODEL_VERSION = "unknown"
                        logger.warning("--- Could not determine model version ---")
                        
            except Exception as client_e:
                logger.warning(f"Warning: Could not fetch model version details from MLflow: {client_e}")
            ...
            logger.warning(f"--- [Attempt {attempt + 1}/{retries}] Error loading model: {load_e} ---")
            
            if attempt < retries - 1:
                logger.info(f"--- Retrying in {delay} seconds... (MLflow server may still be starting) ---")
                time.sleep(delay)
            else:
                # Max retries reached
                logger.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                logger.error(f"--- Max retries ({retries}) reached. Failed to load model at startup. ---")
                logger.error(f"Error: {load_e}")
                logger.error(f"Check if MLflow server ({MLFLOW_TRACKING_URI}) is running and model/alias exist.")
                logger.error("")
                ...
                logger.error("")
                logger.error("API will start, but model needs to be loaded via lazy loading on first /predict call.")
                logger.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                CURRENT_MODEL_VERSION = "unknown"
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        probability_churn = float(probabilities[1]) if len(probabilities) > 1 else float(probabilities[0])
        
        latency_ms = (time.time() - start_time) * 1000
        logger.info(f"Prediction: {prediction_value} | Proba: {probability_churn:.4f} | Version: {CURRENT_MODEL_VERSION} | Latency: {latency_ms:.2f}ms")
        ...
        try:
            ...
            log_data = (
                CURRENT_MODEL_VERSION, prediction_value, 
                request.gender, request.SeniorCitizen, request.Partner, request.Dependents,
                request.tenure, request.PhoneService, request.MultipleLines, request.InternetService, 
                request.OnlineSecurity, request.OnlineBackup,
                request.DeviceProtection, request.TechSupport, request.StreamingTV, request.StreamingMovies, 
                request.Contract, request.PaperlessBilling,
                request.PaymentMethod, request.MonthlyCharges, request.TotalCharges
            )
