...
class ModelLoader:
    def __init__(self, model_name="telco-churn-champion", tracking_uri="http://mlflow-ui:5000", model_alias=None):
        """
        ...
        try:
            if self.model_alias:
                ...
                try:
                    ...
                    logger.info(f"Loading model with alias '{self.model_alias}': {model_uri}")
                    self.model = mlflow.pyfunc.load_model(model_uri)
                except Exception as alias_error:
                    logger.warning(f"Failed to load model with alias '{self.model_alias}': {alias_error}")
                    logger.info(f"Falling back to latest version of model '{self.model_name}'")
            ...
            if not self.model_version:
                try:
                    ...
                    if self.model_alias:
                        ...
                        model_info = client.get_model_version_by_name(self.model_name, self.model_alias)
...
def load_latest_model(model_name="telco-churn-champion", tracking_uri="http://127.0.0.1:5000"):
    """
