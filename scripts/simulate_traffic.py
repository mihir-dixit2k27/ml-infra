...
def send_prediction_request(row):
    ...
    try:
        if 'MonthlyCharges' in data and data['MonthlyCharges'] is not None:
            ...
            print(f"  🔄 Simulating drift: MonthlyCharges {original_value:.2f} -> {data['MonthlyCharges']:.2f} ({drift_factor}x)")
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Prediction: {result['prediction']}, Churn Prob: {result['probability_churn']:.3f}")
            return True
        else:
...
def check_api_health():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ API is healthy - Model loaded: {health_data.get('model_loaded', False)}")
            return True
        else:
